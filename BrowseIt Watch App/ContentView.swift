...
func openWebsite(website urlString: String? = nil, customURL: String? = nil) {
    let target = customURL != nil ? "https://\(customURL!)" : urlString
...
struct ContentView: View {
    var body: some View {
        NavigationStack {
            List {
                Section {
                    TextFieldLink("Custom URL") { url in
                        openWebsite(customURL: url)
                    }
                }
                ...
                Section {
                    ForEach(websiteCategories) { category in
                        NavigationLink(category.title) {
                            WebsitesListView(category: category)
                        }
                    }
                }
            }
...
struct WebsitesListView: View {
    ...
    var body: some View {
        List(category.websites) { website in
            Button(website.name) {
                openWebsite(website: website.url)
            }
        }
